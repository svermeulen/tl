...
tl.type_check = function(ast, opts)
   ...
   local function match_record_key(tbl, rec, key)
      ...
      assert(type(key) == "string", "Unexpected type '" .. tostring(type(key)) .. "'")
         ...
         after = function(node, children)
            ...
            elseif node.op.op == ":" then
               if lax and (is_unknown(a) or a.typename == "typevar") then
                  ...
                  local k = node.e2.conststr or node.e2.tk
                  if not k then
                     node.type = INVALID
                     return node_error(node, "unknown error")
                  else
                     local t, e = match_record_key(a, node.e1, k)
                     if not t then
                        node.type = INVALID
